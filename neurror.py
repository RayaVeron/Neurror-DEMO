...
# In order for our json file to write user_inputs into its dictionary list
data = {"user_input": []}
...
def clean_text(text):  # Define our function as clean_text, that accepts one paremeter: variable text
    ...
    # The Information in the text variable will be turned to lower letters, and blank spaces on both ends will be stripped
    text = text.lower().strip()
    text = text.split()  # Every word in this variable will be split into seperate strings
    # Every string inside the variable will be joined into one string, using the " " as a glue
    text = " ".join(text)
...
# Here we define our function as nrr_chat that accepts a variable as paremeter


def nrr_chat(clean_input):
    # The variable clean_input contains the refined text of user's input

    # Here we declare that the variable Vector
...
def neurror():
   ...
   # Before we can actually apply neurror chat as users
    ...
    # Here we will store our vectors (x,y) from the training data
    emotion_label = []
    # Every sentence will be embedded into 384 dimensional vectors (x)
    emotion_content = []
    ...
    # Here we have our training "classifier", object
    clf = OneVsRestClassifier((LogisticRegression(max_iter=1000, solver="liblinear"))) #OnevsRest protects it from post developer updates
    #
    ...
    # Fit emotion_content/label database into the training model
    clf.fit(emotion_content, emotion_label)

    # -----------------------------
    # LOOP
    # -----------------------------
    while True:
        # As long as user doesn't type x the loop will persist
        ...
        # Refine user's input before sending to the model
        clean_input = clean_text(user_input)

        try:  # this is the code we are going to try and execute
            # Run nrr_chat function using user's input, and store it in the vector (It will be stored as list of decimal integers between 0-1)
            if len(user_input) > 50:
                 raise ValueError("Not allowed more than 50 characters")
            vector = nrr_chat(clean_input)
            # Predict the probability that this sample, in the row 0, belongs to each one of the possible classes (labels)
            probs = clf.predict_proba([vector])[0]
            # Use attribute .classes to store all the data labels, inside the classes variable
            ...
            print("----------------------\n",
                  ...
                  "\n"
                  "----------------------\n")
            # For every label, and its percentage
            for label, pct in zip(classes, probs * 100):
                print(f"{label}: {round(pct)}%")  # Turn it into a string
                print("")

        except ValueError as fail:  # If any error happens
            print(fail)  # Shows the error message
            continue  # Go back to the start of the loop

        # Store cleaned user input in nrrdata.json file
        data["user_input"].append(clean_input)

        # We open the file from storage variable, and hold it as an f for writing in data
        with open(storage, "w") as f:
            # Write the data variable into object f (the real file), use indentation: 4
